...
impl StreamSegmenter {
    ...
    fn should_emit_segment(&self) -> bool {
        ...
        let chinese_char_count = self.buffer.chars()
    ...
    fn clean_markdown(&self, text: &str) -> String {
        ...
        if let Some(dot_pos) = trimmed.find(". ") {
            if dot_pos > 0 && dot_pos <= 3 {  // Check if it's a reasonable number (1-999)
        ...
        for line in lines {
            ...
            if trimmed.starts_with('#') && trimmed.chars().nth(1).map_or(false, |c| c.is_whitespace()) {
...
mod tests {
    ...
    fn test_chinese_character_segmentation() {
        ...
        let long_chinese = "第三鲜是一道传统的中国菜通常以鸡蛋虾仁和黄瓜为主料因其色香味俱佳而得名这道菜"; // 35 characters
        
        // Add the text in chunks
        assert_eq!(segmenter.add_chunk("第三鲜是一道传统的中国菜通常以"), None); // 13 chars
        assert_eq!(segmenter.add_chunk("鸡蛋虾仁和黄瓜为主料"), Some("第三鲜是一道传统的中国菜通常以鸡蛋虾仁和黄瓜为主料".to_string())); // +10 = 23 chars total > 20
        
        // Test Chinese with punctuation - should segment at punctuation
        let mut segmenter2 = StreamSegmenter::new(10);
        assert_eq!(segmenter2.add_chunk("第三鲜是一道传统菜。"), Some("第三鲜是一道传统菜。".to_string()));
    }
    ...
    fn test_markdown_cleaning() {
        ...
        // Test list markers
        assert_eq!(segmenter.clean_markdown("- 在锅中留少量底油"), "在锅中留少量底油");
        ...
        assert_eq!(segmenter.clean_markdown("This is **bold** text"), "This is bold text");
        
        // Test italic markdown (be careful with Chinese text)
        assert_eq!(segmenter.clean_markdown("*italic* text"), "italic text");
        
        // Test headers
        assert_eq!(segmenter.clean_markdown("## Section Title"), "Section Title");
        assert_eq!(segmenter.clean_markdown("### Subsection"), "Subsection");
        
        // Test code blocks
        assert_eq!(segmenter.clean_markdown("Use `function()` here"), "Use function() here");
        
        // Test links
        assert_eq!(segmenter.clean_markdown("Visit [Google](https://google.com)"), "Visit Google");
        
        // Test combined markdown
        assert_eq!(segmenter.clean_markdown("**准备** *材料* 和 `工具`"), "准备 材料 和 工具");
    }
